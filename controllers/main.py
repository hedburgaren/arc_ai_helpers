...
def _get_json_data():
    """Get JSON request data, compatible with Odoo 18 CE.
    In type='json' routes, Odoo parses the JSON-RPC envelope and passes
    params as kwargs to the handler. But we also need to handle direct
    JSON POST bodies for webhook callbacks from n8n."""
    # Try get_json_data first (Odoo 18)
    if hasattr(request, 'get_json_data'):
        data = request.get_json_data()
        if isinstance(data, dict):
            # If it's a JSON-RPC envelope, extract params
            if 'params' in data:
                return data['params']
            return data
...
def _check_bearer_token():
    """Validate Bearer token against res_users_apikeys (Odoo 18 CE compatible).
    Odoo 18 stores API keys hashed with passlib PBKDF2-SHA512."""
    ...
    if not token:
        raise AccessDenied("Empty Bearer token")
    crypt_ctx = CryptContext(schemes=["pbkdf2_sha512"])
    cr = request.env.cr
    cr.execute("SELECT id, user_id, key FROM res_users_apikeys")
    for row in cr.fetchall():
        if crypt_ctx.verify(token, row[2]):
            return row[1]
